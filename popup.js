document.addEventListener('DOMContentLoaded', () => {
  ...
  const keepOpenButton = document.getElementById('keepOpenButton');
  ...
  function createPersistentWindow() {
    // Get the current popup window's position and size
    chrome.windows.getCurrent((currentWindow) => {
      const width = 400;
      const height = 600;
      ...
      chrome.windows.create({
        ...
        // Close the original popup using the Chrome API
        chrome.windows.getCurrent((currentWindow) => {
  ...
  // Handle keep open button
  keepOpenButton.addEventListener('click', () => {
    console.log('Creating persistent window');
